...
ggplot(cambium_temperature %>% filter(cell_viability <2), aes(x = temperature, y = cell_viability))+
  geom_point()+
  geom_smooth(method = "glm",
              method.args = list(c("Gamma")))+
  annotate("text", x = 25, y = 2.25, label = "r2 = 0.27")+
  annotate("text", x = 25, y = 2.1, label = "p = 0.036")+
  theme_classic()

summary(lmer(cell_viability~temperature + (1|tree) + (1|branch), data = cambium_temperature %>% filter(cell_viability <2)))
r.squaredGLMM(lmer(cell_viability~temperature + (1|tree) + (1|branch), data = cambium_temperature %>% filter(cell_viability <2)))
```
