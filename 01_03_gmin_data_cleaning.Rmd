...
```{r}
ggplot(gmin_raw %>% filter(date == "2024-12-04"), aes(x = time, y = weight))+
  geom_point()+
...
enviro <- rbind(data_logger_0, data_logger_1, data_logger_2, data_logger_3, data_logger_4) %>% 
...
gmin <- read.csv(here("data", "raw_data", "gmin", "gmin_master.csv")) %>% 
  ...
  reframe(gmin = mean(gmin)) %>%
         mutate(treatment = case_when(branch == 0 ~ "preburn",
                                      tree < 90 ~ "burned",
                                      tree >= 90 ~ "unburned"),
         species = "quag") %>% 
  drop_na()
